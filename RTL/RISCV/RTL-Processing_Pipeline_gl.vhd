...
entity Pipeline is
  ...
  port (
    ...
    irq_i                      : in  std_logic;
    -- debug signals
    debug_marker               : out std_logic_vector(THREAD_POOL_SIZE-1 downto 0);
...
architecture Pipe of Pipeline is
  ...
  signal MVSIZE_flat                 : std_logic_vector(((THREAD_POOL_SIZE)*(Addr_Width + 1))-1 downto 0);
  signal MVTYPE_flat                 : std_logic_vector(((THREAD_POOL_SIZE)*(4))-1 downto 0);
  signal MPSCLFAC_flat               : std_logic_vector(((THREAD_POOL_SIZE)*(5))-1 downto 0);
  signal dsp_except_data_flat        : std_logic_vector(((ACCL_NUM)*(32))-1 downto 0);
  signal dsp_sc_data_read_flat       : std_logic_vector(((ACCL_NUM)*(2)*(SIMD_Width))-1 downto 0);
  signal dsp_we_word_flat            : std_logic_vector(((ACCL_NUM)*(SIMD))-1 downto 0);
  signal dsp_sc_read_addr_flat       : std_logic_vector(((ACCL_NUM)*(2)*(Addr_Width))-1 downto 0);
  signal dsp_to_sc_flat              : std_logic_vector(((ACCL_NUM)*(SPM_NUM)*(2))-1 downto 0);
  signal dsp_sc_data_write_wire_flat : std_logic_vector(((ACCL_NUM)*(SIMD_Width))-1 downto 0);
  signal dsp_sc_write_addr_flat      : std_logic_vector(((ACCL_NUM)*(Addr_Width))-1 downto 0);
  signal dsp_sci_we_flat             : std_logic_vector(((ACCL_NUM)*(SPM_NUM))-1 downto 0);
  signal dsp_sci_req_flat            : std_logic_vector(((ACCL_NUM)*(SPM_NUM))-1 downto 0);
  signal state_DSP_flat              : std_logic_vector(((ACCL_NUM)*(2))-1 downto 0);
...
------------------------------------------------------------------------------------------------------------------------------------
  ...
  port map(
    ...
    MVSIZE                     => MVSIZE_flat,
    MVTYPE                     => MVTYPE_flat,
    MPSCLFAC                   => MPSCLFAC_flat,
    dsp_except_data            => dsp_except_data_flat,
    ...
    dsp_sc_data_read           => dsp_sc_data_read_flat,
    dsp_sc_read_addr           => dsp_sc_read_addr_flat,
    dsp_to_sc                  => dsp_to_sc_flat,
    dsp_sc_data_write_wire     => dsp_sc_data_write_wire_flat,
    dsp_we_word                => dsp_we_word_flat,
    dsp_sc_write_addr          => dsp_sc_write_addr_flat,
    dsp_sci_we                 => dsp_sci_we_flat,
    dsp_sci_req                => dsp_sci_req_flat,
    state_DSP                  => state_DSP_flat
	);
...
--    write(row, to_string(now) & " "); 
  ...
  -- Flat signals
gen_flatten_in: process(all)
begin

  for i in accl_range loop
    for j in 0 to 1 loop
      dsp_sc_data_read_flat(i*SIMD_Width*2 + (j+1)*SIMD_Width -1 downto i*SIMD_Width*2 + j*SIMD_Width) <= dsp_sc_data_read(i)(j);
    end loop;
  end loop;

  for i in 0 to THREAD_POOL_SIZE-1 loop
    MVSIZE_flat((i+1)*(Addr_Width+1) -1 downto i*(Addr_Width+1)) <= MVSIZE(i);
    MVTYPE_flat((i+1)*4 -1 downto i*4) <= MVTYPE(i);
    MPSCLFAC_flat((i+1)*5 -1 downto i*5) <= MPSCLFAC(i);
  end loop;
end process;

gen_flatten_out: process(all)
begin
  for i in accl_range loop
    dsp_except_data(i)        <= dsp_except_data_flat((i+1)*32 -1 downto i*32);
    dsp_we_word(i)            <= dsp_we_word_flat((i+1)*SIMD -1 downto i*SIMD);
    dsp_sc_data_write_wire(i) <= dsp_sc_data_write_wire_flat((i+1)*SIMD_Width -1 downto i*SIMD_Width);
    dsp_sc_write_addr(i)      <= dsp_sc_write_addr_flat((i+1)*Addr_Width -1 downto i*Addr_Width);
    dsp_sci_we(i)             <= dsp_sci_we_flat((i+1)*SPM_NUM -1 downto i*SPM_NUM);
    dsp_sci_req(i)            <= dsp_sci_req_flat((i+1)*SPM_NUM -1 downto i*SPM_NUM);
    state_DSP(i)              <= state_DSP_flat((i+1)*2 -1 downto i*2);

    for j in 0 to 1 loop
      dsp_sc_read_addr(i)(j) <= dsp_sc_read_addr_flat(i*Addr_Width*2 + (j+1)*Addr_Width -1 downto i*Addr_Width*2 + j*Addr_Width);
    end loop;
    for j in 0 to SPM_NUM-1 loop
