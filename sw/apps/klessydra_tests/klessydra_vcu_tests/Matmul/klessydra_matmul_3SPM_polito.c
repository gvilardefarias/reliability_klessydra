...
int main(){
	...
	if (Klessydra_get_coreID() == 0){
		...
		for (int i = 0; i < n; i++){
			...
			kmemstr(&m_out[th_id][i][0], (void *)((int *)spmaddrB), u * SIZE_OF_INT);
	...
	if (Klessydra_get_coreID() == 0){
		...
		for(int k = 0; k < 3; k++){
			for (int i = 0; i < n; i++){
				for (int j = 0; j < u; j++){
					if(m_out[k][i][j] != ref_mat[i][j]){
						printf("Error in thread %d, row %d, column %d\n", k, i, j);
						printf("Expected: %d, Found: %d\n", ref_mat[i][j], m_out[k][i][j]);
						pass = 0;
						break;
					}
				}
