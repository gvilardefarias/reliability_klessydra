...
then
	...
	USE_KLESSYDRA_M=0       # if you are using klessydra-m (A hart morphing version of klessydra T1), set this to 1CV
	...
	KLESS_morph_en=0                # Enables hardware metamorphosis, that lets the IMT processor morph into an in-order execution processor to avoid stalling
	...
	KLESS_branch_predict_en=0       # Enables a 1-bit branch predictor (if set without btb_en), that predicts a taken branch if the offset is negative, else it preicts a positive offset
	...
	KLESS_SIMD=8                    # Changing the SIMD, would change the number of the functional units in the dsp, and the number of banks in the spms (can be power of 2 only)
	...
	FM_Size=16                       # Defines the size of the Feature Maps in the convolution tests (can only be multiples of 2, FMs > 32 are not tested)
	...
	# File with the list of faults to be injected
	#FAULT_FILE="fault_cntrl_list.txt"
	#FAULT_FILE="fault_fu_all_list_3accl.txt"
	#FAULT_FILE="fault_fu_remain_8simd.txt"
	#FAULT_FILE="fault_fu_all_list_8simd.txt"
	#FAULT_FILE="fault_mul_list_8simd.txt"
	#FAULT_FILE="fault_mul_list.txt"
	#FAULT_FILE="fault_mul_list_3accl.txt"
	#FAULT_FILE="fault_accu_list.txt"
	#FAULT_FILE="fault_accu_list_remain.txt"
	#FAULT_FILE="fault_add_list.txt"
	#FAULT_FILE="fault_fu_wire_list.txt"
	#FAULT_FILE="fault_remain_conv_matmul_list.txt"
	#FAULT_FILE="fault_remain.txt"
	#FAULT_FILE="fault_fu_wire_remain.txt"
	#FAULT_FILE="fault_fu_full_list.txt"
	#FAULT_FILE="fault_fu_list.txt"

	#FAULT_FILE="fault_fu_list_4simd.txt"
	#FAULT_FILE="fault_fu_list_8simd.txt"
	#FAULT_FILE="fault_fu_list_3accl_2simd.txt"
	#FAULT_FILE="fault_fu_list_3accl_4simd.txt"
	#FAULT_FILE="fault_fu_list_3accl_8simd.txt"
	#FAULT_FILE="fault_fu_list_6accl_2simd.txt"
	#FAULT_FILE="fault_fu_list_6accl_4simd.txt"
	FAULT_FILE="fault_fu_list_6accl_8simd.txt"
...
then
	...
	printf "TESTE 000000000000000000000000000000000000"
	OBJDUMP=`which klessydra-unknown-elf-objdump`
	OBJCOPY=`which klessydra-unknown-elf-objcopy`
	COMPILER=`which klessydra-unknown-elf-gcc`
	RANLIB=`which klessydra-unknown-elf-ranlib`
...
let SPME=(2**$KLESS_Addr_Width * 4 + 0x10000000)
let SPMF=(2**$KLESS_Addr_Width * 5 + 0x10000000)
let SPMG=(2**$KLESS_Addr_Width * 6 + 0x10000000)
KLESS_SPM_A="-DKLESS_SPM_A="$SPMA
KLESS_SPM_B="-DKLESS_SPM_B="$SPMB
KLESS_SPM_C="-DKLESS_SPM_C="$SPMC
KLESS_SPM_D="-DKLESS_SPM_D="$SPMD
KLESS_SPM_E="-DKLESS_SPM_E="$SPME
KLESS_SPM_F="-DKLESS_SPM_F="$SPMF
KLESS_SPM_G="-DKLESS_SPM_G="$SPMG
printf '%s' '--'
...
printf $'\n'
printf '%s' '--'
printf " SPME= 0x%x" $SPME
printf $'\n'
printf '%s' '--'
printf " SPMF= 0x%x" $SPMF
printf $'\n'
printf '%s' '--'
printf " SPMG= 0x%x" $SPMG
printf $'\n'
